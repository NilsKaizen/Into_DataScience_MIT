...
def load_cows(filename):
    ...
    with open(filename) as f:
        ...
        c_data_dict = {cow.split(',')[0]: int(
            cow.split(',')[1]) for cow in cow_data}
        f.close()
    return c_data_dict


# print(load_cows('Assigments\PS1\ps1_cow_data.txt'))
# Problem 2
...
def compare_cow_transport_algorithms(cows, limit=10):
    ...
    df = pd.DataFrame(dict_results, columns=[
                      'Algorithm', 'Time', 'Trips', 'Result'])
    return df
...
print(compare_cow_transport_algorithms(
    load_cows('Assigments\PS1\ps1_cow_data.txt'), 10))
